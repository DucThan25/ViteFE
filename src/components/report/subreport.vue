...
export default {
  ...
  methods: {
    async GetReport() {
      this.$_root.context.get('/Cntt/Sub',{fromDate: this.TuNgay, toDate: this.DenNgay}).then((response) => {
        ...
        this.subthongbao = ("Lấy dữ liệu thuê bao thành công!");
        this.$toast.success("Lấy dữ liệu thuê bao thành công!");
        this.kieusubthongbao = 0;
      }).catch((ex)=>{
        this.subthongbao = "Có lỗi khi lấy dữ liệu thuê bao, vui lòng thử lại sau! "+ex.response.data;
        if(ex.response.data)
          this.$toast.error(""+ex.response.data);
        else
          this.$toast.error("Có lỗi khi lấy dữ liệu thuê bao, vui lòng thử lại sau! "+ex);
    ...
    ExportFile() {
      ...
      let csvContent = "data:text/csv;charset=utf-8," 
        +`"Eload","Tên File","Ngày Upload","Số thuê bao","Serial","Kết quả / Mã lỗi"` + this.rows.map(e => `${e.eLoadNumber},"${e.fileName}","${e.uploadDate}","${e.msisdn}","${e.msin}","${e.apiStatus}"`).join("\n");
...
<template>
  <div class="container-fluid">
    <h1>Xem kết quả đăng ký TTTB</h1>
    <div class="row">
