...
export default {
  ...
  methods: {
    async GetReport() {
      this.$_root.context.get('/Cntt/Package',{fromDate: this.TuNgay, toDate: this.DenNgay}).then((response) => {
        ...
        this.subthongbao = ("Lấy dữ liệu gói cước thành công!");
        this.$toast.success("Lấy dữ liệu gói cước thành công!");
        this.kieusubthongbao = 0;
      }).catch((ex)=>{
        this.subthongbao = "Có lỗi khi lấy dữ liệu gói cước, vui lòng thử lại sau! "+ex.response.data;
        if(ex.response.data)
          this.$toast.error(""+ex.response.data);
        else
          this.$toast.error("Có lỗi khi lấy dữ liệu gói cước, vui lòng thử lại sau! "+ex);
    ...
    ExportFile() {
      ...
      let csvContent = "data:text/csv;charset=utf-8," 
        +`"Eload","Tên File","Ngày Upload","Số thuê bao","Mã gói","Kết quả / Mã lỗi"` + this.rows.map(e => `${e.eLoadNumber},"${e.fileName}","${e.uploadDate}","${e.msisdn}","${e.packageCode}","${e.apiStatus}"`).join("\n");
